...
(function(window, document){
  ...
  function createFAB(){
    ...
    const themeBtn = document.createElement('button'); themeBtn.className='ui-fab-btn'; themeBtn.title='Toggle theme'; themeBtn.textContent='☼';
    ...
    wrap.appendChild(themeBtn); wrap.appendChild(sizeBtn); wrap.appendChild(sizeDown);
    document.body.appendChild(wrap);

    themeBtn.addEventListener('click', ()=>{
      const s = loadSettings(); s.theme = (s.theme === 'dark') ? 'light' : 'dark'; applySettings(s); saveSettings(s);
    });
  ...
  function injectControls(){
    ...
    c.innerHTML = `
      <label><input type="checkbox" id="ui-toggle-theme"> Dark theme</label>
      <div style="margin-top:8px"><button id="ui-font-dec">A-</button> <button id="ui-font-inc">A+</button></div>
    `;
    document.body.appendChild(c);
    const chk = c.querySelector('#ui-toggle-theme'); const inc = c.querySelector('#ui-font-inc'); const dec = c.querySelector('#ui-font-dec');
    const s = loadSettings(); chk.checked = s.theme === 'dark';
    chk.addEventListener('change', ()=>{ const cur=loadSettings(); cur.theme = chk.checked ? 'dark' : 'light'; applySettings(cur); saveSettings(cur); });

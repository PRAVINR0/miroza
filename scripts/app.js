...
(function(){
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    try{
      ...
      const isArticleDetail = path.includes('/articles/') && !/articles(\/|index\.html|articles\.html)$/.test(path);
      const isBlogDetail = path.includes('/blogs/') && !/blogs(\/|index\.html|blogs\.html)$/.test(path);
      if(isArticleDetail || isBlogDetail){
        // Inline Article Nav Logic
        (function () {
          'use strict';
          const SELECTORS = {
          ...
          function renderBreadcrumbs() {
            ...
            if (!article || document.querySelector('.breadcrumbs')) return;
          ...
          function renderSocialShare() {
            const article = document.querySelector(SELECTORS.article);
            if (!article || document.querySelector(SELECTORS.shareContainer)) return;
            ...
            const shareHTML = `
              <div class="article-share">
                ...
                <div class="share-buttons">
                  <a href="https://twitter.com/intent/tweet?text=${title}&url=${url}" target="_blank" rel="noopener noreferrer" class="share-btn twitter" aria-label="Share on Twitter"><span>Twitter</span></a>
                  <a href="https://www.facebook.com/sharer/sharer.php?u=${url}" target="_blank" rel="noopener noreferrer" class="share-btn facebook" aria-label="Share on Facebook"><span>Facebook</span></a>
                  <a href="https://www.linkedin.com/shareArticle?mini=true&url=${url}&title=${title}" target="_blank" rel="noopener noreferrer" class="share-btn linkedin" aria-label="Share on LinkedIn"><span>LinkedIn</span></a>
                  <a href="https://api.whatsapp.com/send?text=${title}%20${url}" target="_blank" rel="noopener noreferrer" class="share-btn whatsapp" aria-label="Share on WhatsApp"><span>WhatsApp</span></a>
                </div>
              </div>
            `;
          ...
          async function renderRelatedAndNav() {
            ...
            if (!slug) return;
            try {
              // Use existing store if available to avoid fetch
              let allPosts = [];
              if(window.MIROZA.store && window.MIROZA.store.ready()){
                  allPosts = window.MIROZA.store.getAll();
              } else {
              ...
              if (nextPost || prevPost) {
                const navHTML = `
                  <nav class="article-nav" aria-label="Article navigation">
                    ${prevPost ? `<a href="${prevPost.link || prevPost.url}" class="nav-link prev"><span class="nav-label">&larr; Previous</span><span class="nav-title">${prevPost.title}</span></a>` : '<div class="nav-spacer"></div>'}
                    ${nextPost ? `<a href="${nextPost.link || nextPost.url}" class="nav-link next"><span class="nav-label">Next &rarr;</span><span class="nav-title">${nextPost.title}</span></a>` : '<div class="nav-spacer"></div>'}
                  </nav>
                `;
                ...
                const relatedHTML = `
                  <section class="related-articles">
                    ...
                    <div class="related-grid">
                      ${related.map(p => `<a href="${p.link}" class="related-card"><div class="related-image"><img src="${p.image || '/assets/images/hero-insight-800.svg'}" alt="${p.title}" loading="lazy"></div><h4>${p.title}</h4></a>`).join('')}
                    </div>
                  </section>
                `;
